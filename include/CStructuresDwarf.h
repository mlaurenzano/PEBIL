...
static uint64_t dwarf2_get_leb128_unsigned(char* ptr, uint32_t* len){
    ...
    *len = 0;
    ...
    //fprintf(stdout, "Found unsigned leb128 %lld with %d bytes\n", ret, *len);
    return ret;
}
...
static int64_t dwarf2_get_leb128_signed(char* ptr, uint32_t* len){
    ...
    uint8_t byte;
    uint32_t shift = 0;
    const uint32_t size = sizeof(uint32_t) * 8;
    *len = 0;
    do {
        byte = *ptr;
        ptr++;
    ...
    //    fprintf(stdout, "Found signed leb128 %lld with %d bytes\n", ret, *len);
    return ret;
}
...
{
    uint8_t  cu_length        [4];
    uint8_t  cu_version       [2];
    uint8_t  cu_abbrev_offset [4];
    uint8_t  cu_pointer_size  [1];
}
...
{
    uint8_t  ar_length       [4];
    uint8_t  ar_version      [2];
    uint8_t  ar_info_offset  [4];
    uint8_t  ar_pointer_size [1];
    uint8_t  ar_segment_size [1];
}
...
typedef struct
{
    uint32_t  ar_length;
    uint16_t ar_version;
    uint32_t  ar_info_offset;
    uint8_t  ar_pointer_size;
