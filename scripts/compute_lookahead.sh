...
cat $trace_file | awk '{if ($1 == 0) { print $_ }}' | tr ":" " "  | awk '{for (i = 1; i < 43; i++) { printf $i " "; }  ni=0; ti=0; nf=0; tf=0; for (i=43; i <= NF; i += 3) { vi=i+1; vf=i+2; ni+=$vi; ti+=($i*$vi); nf+=$vf; tf+=($i*$vf); } print ni " " ti " " nf " " tf; }'
